// napi-rs-artifact-metadata:{"version":2,"rootEntry":"index.js","exports":["panic"],"managedRootEntries":["browser.js","index.js","panic.wasm","panic.debug.wasm"]}
/* eslint-disable */
/* prettier-ignore */
...
let __wasmFilePath = __nodePath.join(__dirname, 'panic.wasm32-wasi.wasm')
const __wasmDebugFilePath = __nodePath.join(__dirname, 'panic.wasm32-wasi.debug.wasm')

if (__nodeFs.existsSync(__wasmDebugFilePath)) {
  __wasmFilePath = __wasmDebugFilePath
} else if (!__nodeFs.existsSync(__wasmFilePath)) {
  const __wasiPackageEntry = require.resolve('@napi-rs/wasm-panic-wasm32-wasi')
  const __packagedWasmFilePath = __nodePath.join(__nodePath.dirname(__wasiPackageEntry), 'panic.wasm32-wasi.wasm')
  if (!__nodeFs.existsSync(__packagedWasmFilePath)) {
    throw new Error('@napi-rs/wasm-panic-wasm32-wasi is installed but is missing panic.wasm32-wasi.wasm.')
  }
